...
var promisify = function(nodeStyleFn) {
  return function(){
    var respond = {then:null, catch:null};
    nodeStyleFn(arguments[0], function(error, data){
      if (error) {
        respond.catch = function(callback){callback(error);};
      } else {
        respond.then = function(callback){callback(data);};
      }
      console.log(respond);
      return respond;
    });
